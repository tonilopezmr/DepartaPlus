...
public class UpdateDepart extends JInternalFrame implements Vista {
     ...
     * @throws SQLException
     */
    public int migrarEmpleados(int codigo) throws SQLException {
        //Todos los departamentos
        DepartamentoVo[] departs = controller.departamentosAll();
        //Array auxiliar
        DepartamentoVo[] listaDeps = new DepartamentoVo[departs.length - 1];
        ...
        if (salida != null) {
            DepartamentoVo dep = (DepartamentoVo) salida;
     ...
     * @throws SQLException
     */
    private int traspaso(int codigo, String nombreDep) throws SQLException {
        int delete = 0;
        if (isTraspaso(nombreDep)) {
            int newCod = migrarEmpleados(codigo);
            //controller.traspaso(codigo, newCod);
            if (newCod != -1) {
                delete = controller.bajaDepartamento(codigo, newCod);
            } else {
    ...
    private void baja(int codigo) throws SQLException {
        String nombreDep = controller.nombreDepart(codigo);
        int delete;

        //Si el departamento tiene empleados, los traspasa
        if (controller.hasEmpleados(codigo)) {
            delete = traspaso(codigo, nombreDep);
        } else {
            //Sino lo borra directamente.
            delete = controller.bajaDepartamento(codigo);
        }
        //Si se borro
        if (delete > 0) {
     ...
     * @throws SQLException
     */
    private void modificar(int codigo, String nombre, String loc) throws SQLException {
    ...
    private void consultaModificar(int codigo) throws SQLException {
        if (controller.existeDepartamento(codigo)) {
            mostrarMensaje("** Se puede modificar el "
                    + "departamento " + codigo + " **");
            DepartamentoVo dep = controller.getDepartamento(codigo);
