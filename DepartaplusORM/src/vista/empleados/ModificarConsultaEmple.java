...
public class ModificarConsultaEmple extends JInternalFrame implements Observer, Vista {
    ...
    private JComboBox<Depart> busDepartLista;
    ...
    private void loadModificar() {
        ...
        //Listas
        departamentos = new JComboBox<Depart>();
     ...
     * @param emple
     */
    public void cargarDatosModificar(Emple emple) {
        modNombreField.setText(emple.getApellido());
        ...
        try {
            Depart dep = emple.getDepart();
            for (int i = 0; i < departamentos.getItemCount(); i++) {
                Depart depart = departamentos.getItemAt(i);
        ...
        try {
            for (int i = 0; i < directores.getItemCount(); i++) {
                ...
                if (emple.getDir()== (short)direc.getCodigoDir()) {
     ...
     * @param emple
     */
    public void cargarDatosConsulta(Emple emple) {
        try {
            conCodigoField.setText(emple.getEmpNo()+ "");
            conFechaAltaField.setText(emple.getFechaAlt()+ "");
            conNombreField.setText(emple.getApellido());
            conSalarioField.setText(emple.getSalario() + "");
            conComisionField.setText(emple.getComision() + "");
            conOficioField.setText(emple.getOficio());
            conDepartField.setText(emple.getDepart().getDnombre());
            Director dir = controller.getDirector(emple.getDir());
            if (dir != null) {
    ...
    public void cargarDatosEmple(int codigo) {
        try {
            Emple emple = controller.buscarEmpleado(codigo);
            this.codigo = codigo;
            if (accion.equals(MODIFICAR)) {
    ...
    public void buscarEmpleados() {
        Depart dep = (Depart) busDepartLista.getSelectedItem();
        setRegistrosTabla(busNameField.getText(), (int)dep.getDeptNo());
    }
     ...
     * @param empleados
     */
    public void addRegistros(Emple[] empleados) {
        modeloTabla.setRowCount(0);

        for (int i = 0; i < empleados.length; i++) {
            //Un empleado
            Emple emp = empleados[i];
            //Un array con sus atributos
            Object[] empleado = new Object[NUMCOL];
            empleado[COD] = emp.getEmpNo();
            empleado[NOM] = emp.getApellido();
            empleado[DEP] = emp.getDepart().getDnombre();
    ...
    public void setRegistrosTabla(String nombreEmple, int codDepart) {
        try {
            ...
            if (codDepart != -1) {
                ...
                Emple[] empleados = controller.buscarEmpleadosDepart(nombreEmple, codDepart);

                addRegistros(empleados);
            } else {
                //Busca empleados segun su nombre
                Emple[] empleados = controller.buscarEmpleados(nombreEmple);
    ...
    public void setDepartamentosBuscar() {
        try {            
            ...
            this.busDepartLista.addItem(new Depart((byte)-1, "NONE"));
        }
    }
